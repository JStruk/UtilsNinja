<template>
  <aside
    class="flex h-screen flex-col border-r bg-white transition-all duration-300 ease-in-out dark:bg-slate-900"
    :class="mobile ? 'w-72 max-w-[88vw]' : expanded ? 'w-64' : 'w-20'"
  >
    <div class="p-6 flex items-center justify-between">
      <router-link :to="{ name: 'home' }" class="flex items-center gap-3 overflow-hidden" @click="emit('navigate')">
        ...
        <span v-if="isExpanded" class="font-bold text-xl tracking-tight whitespace-nowrap">Utils<span class="text-primary-600">Ninja</span></span>
      </router-link>
      <button
        v-if="mobile"
        type="button"
        aria-label="Close navigation"
        class="rounded-lg p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 dark:hover:bg-slate-800 dark:hover:text-white"
        @click="emit('close')"
      >
        <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg>
      </button>
    </div>

    <div v-if="isExpanded" class="px-4 mb-4">
    ...
    <nav aria-label="Utilities" class="flex-1 overflow-y-auto px-3 space-y-1 py-4 custom-scrollbar">
      <div v-for="category in filteredCategories" :key="category.name" class="mb-4">
        <h3 v-if="isExpanded" class="px-3 text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">
          {{ category.name }}
        </h3>
        <div class="space-y-1">
          <router-link
            ...
            :aria-label="isExpanded ? undefined : tool.label"
          ...
          >
            ...
            <span v-if="isExpanded" class="text-sm overflow-hidden text-ellipsis whitespace-nowrap">{{ tool.label }}</span>
            <span v-if="!isExpanded" class="absolute left-full ml-4 px-2 py-1 bg-slate-900 text-white text-xs rounded opacity-0 invisible group-hover:opacity-100 group-hover:visible group-focus-visible:opacity-100 group-focus-visible:visible transition-all whitespace-nowrap z-50 shadow-xl">
              {{ tool.label }}
            </span>
          </router-link>
        </div>
    ...
    <div class="p-4 border-t space-y-2">
      ...
      >
        ...
        <span v-if="isExpanded" class="text-sm">{{ isDark ? 'Light Mode' : 'Dark Mode' }}</span>
      </button>
...
const isExpanded = computed(() => mobile || expanded.value)
