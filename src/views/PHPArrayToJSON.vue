<template>
  <TwoPaneLayout>
    <template v-slot:left-pane>
      <div class="flex flex-1 dark:bg-gray-700">
        <div class="flex-grow relative">
          ...
          />
          <div v-if="error" class="absolute bottom-0 left-0 right-0 bg-red-100 text-red-700 p-2 text-sm dark:bg-red-900 dark:text-red-300">
            {{ error }}
          </div>
        </div>
      </div>
    ...
    <template v-slot:right-pane>
      <vue-json-pretty
          ...
          showIcon
          :data="json"
          class="flex-1 dark:bg-gray-900 dark:text-purple-200"
      />
